// Optimized Dashboard.jsx with memoization
...
const DashboardHeader = React.memo(({ dateRange, onRangeChange, darkMode }) => (
  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
      <h1 className={`text-2xl md:text-3xl font-bold tracking-tight ${darkMode ? 'text-text-dark-primary' : 'text-gray-900'}`}>
        Dashboard Overview
      </h1>
      <p className={`${darkMode ? 'text-text-dark-secondary' : 'text-gray-500'} mt-1.5`}>
        Track your music streaming metrics and performance
      </p>
    </div>
    <DateRangeSelector
...
// Extracted to avoid re-renders for all cards when one value changes
...
// Dashboard skeleton is also memoized to prevent re-renders
const DashboardSkeleton = React.memo(({ darkMode }) => {
  return (
...
const Dashboard = () => {
    ...
    // Memoize metrics to prevent recalculation on each render
    ...
    return (
        <div className="space-y-8">
            ...
            {/* Metrics Grid */}
            <MetricCardGrid metrics={metrics} darkMode={darkMode} />
